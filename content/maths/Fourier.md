...
- Fourier series: a (sufficiently nice) periodic function can be expressed as an (infinite) sum of complex exponentials. In other words, $e^{ki\pi x}$, for $k\in\Z$, is a basis for periodic functions.
...
But if an operator $A$ is skew-self-adjoint, then $iA$ must be self-adjoint (since $\langle iA x, y \rangle = -\langle  x, (iA)*y \rangle = \langle  x, Ay \rangle$).
...
Consider the operator $M$, such that $N(f)(x)=f(-x)$. Then $1$ and $-1$ are eigenvalues, and the corresponding eigenspaces are the even and odd functions.
